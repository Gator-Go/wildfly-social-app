...
<div class='pojo-section-header'>Posting Edit Form</div>
<div class='pojo-section-body'>
<div align="left">

    <c:set var="posting" value="${posting}"/>
    <form id="idPostingEdit" name="Posting" action="do" method="POST">

        <p>
            <label style="color: red;width: 100%;text-align: left;">${errorMessage}</label> 
        </p>

        <input type='hidden' id="opPostingEdit" name='op' value="${opAction}">
        <input type='hidden' name='id' value="${posting.id}">
        <input type='hidden' name='deviceId' value="${posting.deviceId}">
        <input type='hidden' name='postingId' value="${posting.postingId}">
        ...
        <table class="pojo_tablestyle">
            ...
            <tr>
              ...
              <c:if test="${!empty postingInfoError}">
                <brk/><label style="color: red; width: 130%;text-align: left;">${postingInfoError}</label>
              </c:if>
              </td>
            </tr>
        ...
        <c:if test="${empty posting.postingTempName && posting.postingType eq 'pic'}">
          <img src="<%= httpRoot %>/thumbnail/Posting/Posting/${posting.cloudId}PicThumb${posting.postingVersion}.jpg" alt="Posting">
        </c:if>
        <c:if test="${empty posting.postingTempName && posting.postingType eq 'vid'}">
          <img src="<%= httpRoot %>/thumbnail/Posting/Posting/${posting.cloudId}VidThumb${posting.postingVersion}.jpg" alt="Posting">
        </c:if>
        <c:if test="${!empty posting.postingTempName && posting.postingType eq 'pic'}">
        ...
        <p>
            <button class='btnEdit' id="edit" type="button" onclick='goPostingEdit("${opAction}");'>Edit</button>
            &nbsp;&nbsp;&nbsp;
            <button class='btnBack' id="back" type="button" onclick='goPostingEdit("${opBack}");'>Back</button>
        </p>
...
<script>
  function goPostingEdit(op)
  {
    $('#opPostingEdit').val(op);
    $('#idPostingEdit').submit();
  }
